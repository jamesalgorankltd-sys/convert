...
async function extractCandidatesWithHeadlessBrowser(pageUrl) {
  ...
  try {
    ...
    const exactRanked = ranked.filter(x => candidateMatchesExactPage(x.url, pageUrl, x.hint));
...
export default async function handler(req, res) {
  try {
    ...
    let useful = filterUsefulImageCandidates(candidates, url);
    const screenshots = dataImageCandidates(useful);
    const strict = strictExactPageFilter(useful.filter(u => !/^data:image\//i.test(String(u || ''))), url);
    candidates = rankForOriginalPage(strict, url).concat(screenshots);
