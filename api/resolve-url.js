...
function cleanCandidate(u) {
  ...
  // Some reader/markdown fallbacks can return a broken combined string like:
  // https://page.htm%5D(https://related-page...). Cut at markdown link joiners so
  // a related page can never be tried as the user's requested image.
  u = u.replace(/%5[Dd]\(https?:\/\/.+$/i, '');
  u = u.replace(/\]\(https?:\/\/.+$/i, '');
  u = u.replace(/\)\(https?:\/\/.+$/i, '');
  u = u.replace(/[\s"'<>\)\]\}]+$/g, '');
  return u;
...
async function extractCandidatesWithHeadlessBrowser(pageUrl) {
  ...
  try {
    ...
    await page.goto(originalPageUrl, { waitUntil: 'domcontentloaded', timeout: 45000 }).catch(async (e) => {
      await page.goto(fetchPageUrl, { waitUntil: 'domcontentloaded', timeout: 45000 });
    });
    try { await page.waitForNetworkIdle({ idleTime: 900, timeout: 9000 }); } catch (_) {}
    try {
      await page.evaluate(() => {
        const buttons = Array.from(document.querySelectorAll('button, [role="button"], a'));
        const btn = buttons.find(b => /accept|agree|allow|close|got it|ok/i.test((b.textContent || b.getAttribute('aria-label') || '').trim()));
        if (btn) btn.click();
      });
      await new Promise(r => setTimeout(r, 700));
    } catch (_) {}
    try { await page.evaluate(() => window.scrollTo(0, Math.floor(document.body.scrollHeight * 0.18))); await new Promise(r => setTimeout(r, 1200)); } catch (_) {}
    ...
    let ranked = (data || [])
      ...
      .filter(x => !/%5[Dd]\(|\]\(/i.test(x.url))
      .filter(x => { try { return !/\.(html?|php|aspx?)$/i.test(new URL(x.url).pathname || ''); } catch (_) { return true; } })
    ...
    async function samePageScreenshotCandidate() {
      // Try to crop the main visual from the exact pasted page. Magnific often hides the
      // real image URL behind JS/CSS/canvas or blocks the media URL, so screenshot is safest.
      let clip = await page.evaluate(() => {
        const bad = /logo|icon|avatar|sprite|placeholder|tracking|pixel|ads|banner|cookie/i;
        const vw = window.innerWidth || 1365, vh = window.innerHeight || 900;
        const getBg = (el) => { try { return getComputedStyle(el).backgroundImage || ''; } catch(e) { return ''; } };
        const candidates = [];
        const push = (el, bonus=0) => {
          if (!el || !el.getBoundingClientRect) return;
          const r = el.getBoundingClientRect();
          const txt = ((el.getAttribute && (el.getAttribute('alt') || el.getAttribute('title') || el.getAttribute('aria-label') || el.className || el.id || el.src || '')) || '').toString().toLowerCase();
          if (bad.test(txt)) return;
          if (r.width < 180 || r.height < 140) return;
          const visibleW = Math.min(vw, r.right) - Math.max(0, r.left);
          const visibleH = Math.min(vh, r.bottom) - Math.max(0, r.top);
          if (visibleW < 180 || visibleH < 140) return;
          const area = visibleW * visibleH;
          if (area < 40000) return;
          let score = area + bonus;
          const bg = getBg(el);
          if (/url\(/i.test(bg)) score += 900000;
          if (/img|picture|photo|image|media|main|preview|figure|canvas|video/i.test(txt + ' ' + el.tagName)) score += 500000;
          // Prefer items not too far down the page and not pure full-page wrappers.
          if (r.top >= -20 && r.top < vh * 0.85) score += 250000;
          if (visibleW > vw * 0.92 && visibleH > vh * 0.92) score -= 700000;
          candidates.push({ x:Math.max(0,r.left), y:Math.max(0,r.top), width:visibleW, height:visibleH, area, score });
        };
        document.querySelectorAll('img,picture,source,canvas,video,figure,[style],div,section,a').forEach((el) => {
          const tag = (el.tagName || '').toLowerCase();
          let bonus = /^(img|picture|canvas|video|figure)$/.test(tag) ? 800000 : 0;
          if (/url\(/i.test(getBg(el))) bonus += 700000;
          push(el, bonus);
        });
        candidates.sort((a,b)=>b.score-a.score);
        const r = candidates[0];
        if (r) return { x:r.x, y:r.y, width:Math.max(1, Math.min(vw - r.x, r.width)), height:Math.max(1, Math.min(vh - r.y, r.height)) };
        return null;
      });
      if (!clip || !clip.width || !clip.height) {
        // Final fallback: crop the central/upper area of the exact page instead of saying
        // no image found. This prevents related/premium links and still captures visible page visual.
        clip = await page.evaluate(() => {
          const vw = window.innerWidth || 1365, vh = window.innerHeight || 900;
          const w = Math.min(1100, Math.floor(vw * 0.86));
          const h = Math.min(720, Math.floor(vh * 0.78));
          return { x: Math.max(0, Math.floor((vw - w) / 2)), y: Math.max(0, Math.floor(vh * 0.08)), width: w, height: h };
        });
      }
      if (clip && clip.width && clip.height) {
    ...
    // Magnific blocks many extracted media URLs with 403 and often exposes related item links.
    // For Magnific page links, prefer a screenshot of the largest image visible on the exact
    // pasted page, so the app never downloads a different related image/video page.
    if (/magnific\.com\//i.test(originalPageUrl)) {
      try { const shot = await samePageScreenshotCandidate(); if (shot) return [shot]; } catch (_) {}
    }
    ...
    // Last-resort same-page fallback for any protected page.
    try { const shot = await samePageScreenshotCandidate(); if (shot) return [shot]; } catch (_) {}
...
function exactPageScreenshotUrls(pageUrl) {
  const base = noHashUrl(pageUrl);
  const enc = encodeURIComponent(base);
  return [
    // Exact pasted-page screenshot fallback. This does not follow related/premium links.
    // It is only used when the site hides/blocks real media URLs.
    `https://s.wordpress.com/mshots/v1/${enc}?w=1400`,
    `https://image.thum.io/get/width/1400/crop/900/noanimate/${base}`
  ];
}
...
function filterUsefulImageCandidates(arr, pageUrl) {
  return unique(arr || []).filter(u => {
    ...
    try { if (/\.(html?|php|aspx?)$/i.test(new URL(s.replace(/[\s"'<>\)\]\}]+$/g,'')).pathname || '')) return false; } catch (_) { if (/\.(html?|php|aspx?)(?:[?#].*)?$/i.test(s.replace(/[\s"'<>\)\]\}]+$/g,''))) return false; }
    if (/%5[Dd]\(|\]\(/i.test(s)) return false;
    if (/magnific\.com\/.+(free-ai-image|premium-video)\//i.test(s)) return false;
...
export default async function handler(req, res) {
  try {
    ...
    // Protected exact-page fallback: never try related/premium URLs, but still return
    // a usable image by capturing the exact pasted page through public screenshot services.
    // This fixes sites like Magnific that expose the page publicly but block media fetches.
    if (!candidates.length && /magnific\.com\//i.test(originalUrl)) {
      candidates = exactPageScreenshotUrls(originalUrl);
    }

    if (!candidates.length) return res.status(200).json({ ok: false, error: 'No image found on exact page. Try opening the page, right-click/copy image address, or drag/drop the image directly.', candidates: [] });
