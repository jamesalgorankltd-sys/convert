  ...
  try {
    ...
    async function samePageScreenshotCandidate() {
      const clip = await page.evaluate(() => {
        ...
        const r = els[0];
        return { x: Math.max(0, r.x), y: Math.max(0, r.y), width: Math.max(1, Math.min(window.innerWidth - Math.max(0, r.x), r.width)), height: Math.max(1, Math.min(window.innerHeight - Math.max(0, r.y), r.height)) };
      });
