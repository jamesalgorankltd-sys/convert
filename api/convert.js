...
function nestedDirectImageFromInput(input) {
  const decoded = decodeDeep(input || '');
  const candidates = [];
  try {
    const u = new URL(decoded);
    for (const [k, v] of u.searchParams.entries()) {
      const d = decodeDeep(v);
      if (isHttp(d) && IMAGE_EXT.test(new URL(d).pathname + new URL(d).search)) candidates.push(d);
      for (const m of d.match(/https?:\/\/[^\s"'<>]+/gi) || []) {
        const c = cleanUrl(m);
        if (isHttp(c) && IMAGE_EXT.test(new URL(c).pathname + new URL(c).search)) candidates.push(c);
      }
    }
  } catch (_) {}
  for (const m of decoded.match(/https?:\/\/[^\s"'<>]+/gi) || []) {
    const c = cleanUrl(m);
    try { if (isHttp(c) && IMAGE_EXT.test(new URL(c).pathname + new URL(c).search)) candidates.push(c); } catch (_) {}
  }
  return uniq(candidates).find(u => !BAD.test(u)) || '';
}
...
async function resolve(input) {
  ...
  // HD fix: Magnific/Freepik viewer URLs often contain the original image URL in query=.
  // Vercel cannot open a rendered tab like the extension, so use this direct HD image first.
  const nestedDirect = nestedDirectImageFromInput(url);
  if (nestedDirect && !isWrongMagnificCandidate(nestedDirect, url, nestedDirect)) return nestedDirect;
