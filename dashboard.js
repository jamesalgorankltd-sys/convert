...
function isPageLikeUrl(u){
  const s = decodeDeep(String(u||'')).trim().replace(/&amp;/g,'&').replace(/[\s"'<>\)\]\}]+$/g,'');
  try{
    const x = new URL(s);
    return /\.(html?|php|aspx?)$/i.test(x.pathname || '');
  }catch(e){
    return /\.(html?|php|aspx?)(?:[?#].*)?$/i.test(s);
  }
}
...
async function fetchImageAsBlobBest(url){
  if(/^data:image\//i.test(String(url||''))) return dataUrlToBlob(url);
  try{
    const b = await fetchViaBackground(url, false);
    if((b.type||'').startsWith('image/')) return b;
    throw new Error('Downloaded URL is not an image: ' + (b.type || 'unknown content-type'));
...
*/
function uniqueList(arr){ const out=[]; const seen=new Set(); (arr||[]).forEach(x=>{ x=decodeDeep(String(x||'')).trim().replace(/&amp;/g,'&').replace(/[\s"'<>\)\]\}]+$/g,''); if(x && !seen.has(x)){seen.add(x); out.push(x);} }); return out; }
