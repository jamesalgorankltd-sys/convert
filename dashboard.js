...
function nestedHdImageFromInput(input){
  const direct = extractDirectImageUrlFromAnyInput(input);
  return direct && isLikelyDirectImageUrl(direct) ? direct : '';
}
...
async function resolveInputUrlForUpload(url){
  ...
  // HD fix for Vercel/web app: Magnific/Freepik viewer URLs often carry the real full-size image in query=.
  // Use it before HTML/backend thumbnail extraction so the CDN upload keeps original pixels.
  const nestedHd = nestedHdImageFromInput(url);
  if(nestedHd && !isWrongMagnificCandidate(nestedHd, url, nestedHd)) return nestedHd;

  // Browser extension mode can still read the rendered visible image when needed.
  try{
...
function webpCloudinaryUrl(url){
  ...
  if(url.includes('/image/upload/')){
    ...
    // q_100 keeps maximum WebP quality. No resize/crop/e_improve/e_sharpen, so details are not softened.
