...
async function resolveInputUrlForUpload(url){
  ...
  // For normal page links, encoded direct image values are helpful.
  // For Magnific pages, hash/query can be an old/search/reference image, so rendered page
  // extraction must run first to avoid returning a different picture.
  if(!isMagnificPage){
    const nestedDirectFirst = extractDirectImageUrlFromAnyInput(url);
    if(nestedDirectFirst && !isWrongMagnificCandidate(nestedDirectFirst, url)) return nestedDirectFirst;
  }
